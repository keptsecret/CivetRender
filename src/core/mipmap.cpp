...
MIPMap<T>::MIPMap(const Point2i& res, const T* img, bool do_trilinear, float max_aniso, ImageWrap wrap_mode) :
	...
	if (!isPowerOf2(resolution[0]) || !isPowerOf2(resolution[1])) {
					...
					for (int s = 0; s < res_pow2[0]; s++) {
						...
						for (int j = 0; j < 4; j++) {
							...
							if (wrap_mode == ImageWrap::Repeat) {
								...
								orig_s = clamp(orig_s, 0, resolution[0] - 1);
							}
		...
		for (int i = 1; i < n_levels; i++) {
			...
			pyramid.reset(new BlockedArray<T>(sres, tres));
...
T MIPMap<T>::lookup(const Point2f& st, Vector2f dst0, Vector2f dst1) const {
	...
	if (minorLength * max_anisotropy < majorLength && minorLength > 0) {
		float scale = majorLength / (minorLength * max_anisotropy);
		dst1 *= scale;
		minorLength *= scale;
	}
	if (minorLength == 0) {
	...
	float lod = std::max((float)0, levels() - 1.f + log2(minorLength));
